...
from easy.fetcher.cache import get_the_latest_chapter, cache_easy_search_ranking, cache_others_search_ranking
...
# jinjia2 config
env = Environment(
    loader=PackageLoader('easy.views.md_blueprint', '../templates/md'),
...
async def admin_setting(request):
    ...
    if user:
        try:
            ...
            if data:
                return template('admin_setting.html', title='{user}的设置 - easy'.format(user=user),
                                is_login=1,
                                user=user,
...
async def book_list(request):
    ...
    if user:
        try:
            return template('admin_book_list.html', title='{user}的书单 - easy'.format(user=user),
                            is_login=1,
                            user=user)
        except Exception as e:
            LOGGER.error(e)
        ...
        try:
            ...
            if data:
                ...
                if bookmarks:
                    ...
                    return template('admin_bookmarks.html', title='{user}的书签 - easy'.format(user=user),
                                    is_login=1,
                                    user=user,
                                    is_bookmark=1,
                                    result=result[::-1])
            ...
            if data:
                ...
                if books_url:
                    ...
                    for i in books_url:
                        ...
                        latest_data = await motor_db.latest_chapter.find_one({'easy_chapter_url': book_url})
                        if latest_data:
                            item_result['latest_chapter_name'] = latest_data['data']['latest_chapter_name']
                            item_result['easy_content_url'] = latest_data['data']['easy_content_url']
                        else:
                            ...
                            item_result['easy_content_url'] = get_latest_data.get(
                                'easy_content_url', '')
                        item_result['add_time'] = i.get('add_time', '')
                        ...
                        result.append(item_result)
                    return template('admin_books.html', title='{user}的书架 - easy'.format(user=user),
                                    is_login=1,
                                    user=user,
                                    is_bookmark=1,
                                    result=result[::-1])
...
async def index(request):
    ...
    if user:
        return template('index.html', title='easy', is_login=1, user=user, search_ranking=search_ranking,
                        first_type=first_type, first_type_title=first_type_title, novels_head=novels_head, is_owl=1)
    else:
        return template('index.html', title='easy', is_login=0, search_ranking=search_ranking, first_type=first_type,
    ...
    if user:
        try:
            ...
            if data:
                ...
                return template('noti_book.html', title='新书提醒 - easy'.format(user=user),
                                is_login=1,
                                is_author=is_author,
