...
from easy.fetcher.extract_novels import extract_chapters
from easy.fetcher.function import get_time, get_netloc
from easy.fetcher.decorators import authenticator, auth_params, response_handle
from easy.fetcher import UniResponse, ResponseField
from easy.fetcher.cache import cache_easy_novels_chapter
from easy.fetcher.novels_tools import get_novels_info
from easy.config import LOGGER

api_bp = Blueprint('api_blueprint', url_prefix='api')


@api_bp.route("/owl_bd_novels/<name>")
@authenticator('easy-Api-Key')
...
async def owl_novels_chapters(request, **kwargs):
    ...
    try:
        res = await cache_easy_novels_chapter(url=chapters_url, netloc=netloc)
        chapters_sorted = []
        if res:
