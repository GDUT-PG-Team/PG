...
from Easy.database.mongodb.mongoDB import MotorBase
from Easy.fecher.cache import get_the_latest_chapter
from Easy.config import LOGGER, CONFIG

admin_bp = Blueprint('admin_blueprint', url_prefix='admin')
admin_bp.static('/front/static/novels', CONFIG.BASE_DIR + 'front/static/novels')
...
# jinjia2 config
env = Environment(
    loader=PackageLoader('Easy.function.views.novels_blueprint', '../templates/novels'),
                ...
                if books_url:
                    ...
                    for i in books_url:
                        ...
                        if latest_data:
                            ...
                            item_result['Easy_content_url'] = get_latest_data.get('Easy_content_url', '')
                        item_result['add_time'] = i.get('add_time', '')
        ...
        try:
            ...
            if books_url:
                ...
                for i in books_url:
                    ...
                    item_result["last_read_chapter_name"] = last_read_chapter_name
                    result.append(item_result)
                return template('search_user.html', title='{name}的书架 - Easy'.format(name=name),
                                is_login=1,
                                user=user,
                                username=name,
                                is_bookmark=1,
