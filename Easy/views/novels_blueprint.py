...
from easy.config import RULES, LOGGER, REPLACE_RULES, ENGINE_PRIORITY, CONFIG
from easy.database.mongodb import MotorBase
from easy.fetcher.cache import cache_easy_novels_content, cache_easy_novels_chapter, \
    cache_easy_search_ranking
from easy.fetcher.function import get_time, get_netloc
from easy.fetcher.novels_tools import get_novels_info
from easy.utils import ver_question
...
# jinjia2 config
env = Environment(
    loader=PackageLoader('easy.views.novels_blueprint', '../templates/novels'),
...
async def chapter(request):
    ...
    content = await cache_easy_novels_chapter(url=url, netloc=netloc)
    if content:
...
@novels_bp.route("/easy_donate")
async def donate(request):
    return template('donate.html')


@novels_bp.route("/easy_feedback")
...
async def index(request):
    ...
    search_ranking = await cache_easy_search_ranking()
    if user:
        return template('index.html', title='easy - 网络小说搜索引擎', is_login=1, user=user,
                        search_ranking=search_ranking[:25])
    else:
        return template('index.html', title='easy - 网络小说搜索引擎', is_login=0, search_ranking=search_ranking[:25])


@novels_bp.route("/easy_content")
async def easy_content(request):
    """
    返回小说章节内容页
    : content_url   这决定当前U页面url的生成方式
    : url           章节内容页源url
    ...
    if url == chapter_url:
        ...
        if user and is_ajax == "owl_cache":
            owl_referer = request.headers.get('Referer', '').split('easy_content')[1]
            if owl_referer:
                latest_read = "/easy_content" + owl_referer
    ...
    content_url = RULES[netloc].content_url
    content_data = await cache_easy_novels_content(url=url, netloc=netloc)
    if content_data:
        try:
            ...
            if user:
                ...
                if book:
                    ...
                    if is_ajax == "owl_cache":
                        owl_referer = request.headers.get('Referer', bookmark_url).split('easy_content')[1]
                        latest_read = "/easy_content" + owl_referer
                        await motor_db.user_message.update_one(
                            {'user': user, 'books_url.book_url': book_url},
                            {'$set': {'books_url.$.last_read_url': latest_read}})
...
async def easy_register(request):
    """
    用户登录
    :param request:
    :return:
